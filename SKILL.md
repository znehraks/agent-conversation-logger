...
- Live Markdown: `<codex-output>/codex/<session_id>/transcript.md` (one directory per session — additional artifacts can be added alongside `transcript.md` later)
...
- Default Obsidian symlink: `<vault>/agent-logs/codex-logs` → `<codex-output>/codex/`. Override with `--obsidian-link`, `CODEX_SESSION_EXPORTER_OBSIDIAN_LINK`, or by setting `OBSIDIAN_VAULT`.
...
- Live Markdown: `<claude-output>/claude-code/<session_id>/transcript.md` (one directory per session)
...
- Default Obsidian symlink: `<vault>/agent-logs/claude-logs` → `<claude-output>/claude-code/`. Override with `--claude-obsidian-link`, `CLAUDE_LOGGER_OBSIDIAN_LINK`, or by setting `OBSIDIAN_VAULT`.

The two symlinks land under a shared `agent-logs/` parent so Codex and Claude Code transcripts sit side by side in Obsidian.
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/install.py" \
  --obsidian-link "$OBSIDIAN_VAULT/agent-logs/codex-logs" \
  --claude-obsidian-link "$OBSIDIAN_VAULT/agent-logs/claude-logs"
```
...
- Live log: append-only transcript for immediate review in Obsidian. Codex writes to `<codex-output>/codex/<session>/transcript.md`; Claude Code writes to `<claude-output>/claude-code/<session>/transcript.md`. Each session has its own directory so additional per-session artifacts can join `transcript.md` later. The two output roots are independent.
